...
internal class DispatchTask<in T>(
    ...
    override fun run() {
        ...
        withCoroutineContext(context) {
            when {
                job != null && !job.isActive -> continuation.resumeWithException(
                        job.getCompletionException())
                exception -> continuation.resumeWithException(
                        value as Throwable)
...
internal class DispatchedContinuation<in T>(
    ...
    override fun resume(value: T) {
        ...
        if (dispatcher.isDispatchNeeded(context))
            dispatcher.dispatch(context,
                    DispatchTask(continuation, value, exception = false,
                            cancellable = false))
        else
    ...
    override fun resumeWithException(exception: Throwable) {
        ...
        if (dispatcher.isDispatchNeeded(context))
            dispatcher.dispatch(context,
                    DispatchTask(continuation, exception, exception = true,
                            cancellable = false))
        else
    ...
    internal fun dispatchYield(value: T) {
        val context = continuation.context
        dispatcher.dispatch(context,
                DispatchTask(continuation, value, false, true))
    }
